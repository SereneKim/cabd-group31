...
To vaccinate or not has been a recurring discussion since the beginning of the COVID-19 vaccine. Fear, doubts, and suspicions around the vaccine have appeared not only among the general citizens but also among the medical workers such as doctors and nurses \[e.g., \]. This slightly contrasts the observation that the anti-vaccination sentiment often goes hand in hand with the mistrust of the government and the medical industry [@rothstein2015vaccines]. Then, what does actually make the anti-vaccination sentiment or vaccine hesitancy during the COVID-19 crisis?

We investigate the characteristics of vaccine hesitancy with two sub-questions. First, with the **Twitter** data from 01 October 2020 to 31 March 2021, we will explore *which topics were associated with the anti-vaccination sentiment using an unsupervised topic model*. Here, the LDA method will be used to construct the topic model. This analysis allows a glimpse of the characteristics of anti-vaccination sentiment - namely, where the negative sentiments about COVID-19 vaccination arise. With the insights we gather from the first step, we further analyze *the characteristics of vaccine hesitancy* with survey data, **Living, working and COVID-19 Wave 3.** As this data was collected between 15 February 2021 and 30 March 2021, we will be able to combine the insights with the ones from the Twitter data. This survey data provide responses to more specific questions regarding one's attitude towards COVID-19 vaccines and sociodemographic data about the respondents, which are not available in the Twitter data. This richness of the data serves us to draw a better classification model to discover what makes one hesitant to be vaccinated. Here, we utilize supervised machine learning methods such as a decision tree and random forest.

Note that as the Twitter data we will use are collected based on a list of anti-vaxxer keywords, the term *anti-vaccination sentiment* may be more proper for Study 1. Nevertheless, we assume that one's anti-vaccination sentiment reflects vaccine hesitancy, as one will be hesitant to vaccinate when they have a negative view of vaccination. Therefore, they will use the terms anti-vaccination sentiment and vaccine hesitancy interchangeably in this project.

Lastly, throughout the project, we aim to explore two types of 'big' data - Twitter data and (big) survey data. As we learned about their strong- and weak points from the literature review (Assignment 1), we will analyze the two types of data with the same research question in mind. We hope to discover more aspects of vaccine hesitancy with the two data.

# Table of Contents

```{r toc, echo=FALSE, warning=FALSE, message=FALSE}
render_toc("Group_31_Final.Rmd")
```
